...
describe("PSI", () => {
  it("should compute intersection size on MinHash signatures", async () => {
    ...
    const originalOverlapPercentage =
      (commonItems.length / aliceOriginalItems.length) * 100;
    ...
    const server = await PSIServer.create();
    const client = await PSIClient.create();
    ...
    console.log(
      "Original items overlap:",
      `${commonItems.length}/${
        aliceOriginalItems.length
      } (${originalOverlapPercentage.toFixed(1)}%)`
    );
    ...
    expect(matchPercentage).toBeGreaterThanOrEqual(40); // At least 40%
  ...
  it("should compute intersection size on raw items", async () => {
    ...
    console.log(
      "Match percentage:",
      matchPercentage.toFixed(1) + "% (of Bob's items)"
    );

    // With raw items, we expect ~80% match (4 out of 5 items match)
    // Allow small margin for PSI false positive rate
    const expectedMatchPercentage =
      (expectedIntersection / bobItems.length) * 100;
